...
def main():
    parser = argparse.ArgumentParser(description="Conv2D + LSTM")

    # Dataset
    parser.add_argument("--dataset", type=str, default="mnist", choices=["mnist", "fashion-mnist", "cifar-10"])
    ...
    parser.add_argument("--hidden_dim", type=int, default=128, help="Dimension of the hidden size.")
    # Misc
    ...
    trainloader, testloader, input_shape, classes = load_dataset(args.dataset, args.batch_size, use_gpu,
                                                                 args.num_workers)
    model = build_model(args.model, input_shape, args.hidden_dim, len(classes))
    ...
    with SummaryWriter(log_dir) as writer:
        for epoch in range(args.epochs):
            train(model, trainloader, criterion, optimizer, use_gpu, writer, epoch)

            if epoch % args.eval_freq == 0 or epoch == args.epochs - 1:
                eval(model, testloader, criterion, scheduler, use_gpu, writer, epoch)
